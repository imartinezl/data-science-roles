...
def get_urls(base_url, params, num_pages):
    ...
    # assert len(urls) == num_pages * 10, "There are missing job links"
    return urls
...
def get_data(base_url, params, num_pages=1):
    ...
    Returns:
        posts: Python dict including all posting data
    ...
    if isinstance(urls, list):
        ...
        for i, url in enumerate(urls):
            try:
                ...
                posts[i] = {}
                posts[i]['title'], posts[i]['posting'], posts[i]['url'] = title, posting, url
            except: 
                continue
